...
<template>
  ...
  <n-form ref="formRef" label-placement="left" label-width="auto">
    <n-grid x-gap="12" :cols="2" style="justify-items: start;">
      <template />
      <n-grid-item v-for="item of display_config" :key="item" style="margin-left: 15px;">
        <n-form-item :label="item">
          <n-input-number v-if="typeof (default_store.user_config[item].data) === 'number'"
            v-model:value="default_store.user_config[item].data" />
          <n-input v-else v-model:value="default_store.user_config[item].data" />
        </n-form-item>
      </n-grid-item>
    </n-grid>
  </n-form>
