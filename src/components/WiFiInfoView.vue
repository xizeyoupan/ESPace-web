...
const { isOnline, host, sta_ssid, sta_pass, } = storeToRefs(device)
...
<template>
    <n-flex justify="space-between">
        <h2>当前连接信息</h2>
        <n-button strong secondary type="primary" @click="get_wifi_info">刷新</n-button>
    </n-flex>

    <div><span class="label">WiFi模式:</span>{{ `${wifiModes[wifi_info.WiFi_mode]}` }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">SSID:</span>{{ wifi_info.SSID }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">RSSI:</span>{{ wifi_info.RSSI }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">Channel:</span>{{ wifi_info.channel }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">本机ip:</span>{{ wifi_info.ip }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">网关ip:</span>{{ wifi_info.gw }}</div>
    <div v-if="wifi_info.WiFi_mode === 1"><span class="label">子网掩码:</span>{{ wifi_info.netmask }}</div>
    ...
    <n-modal v-model:show="showModal" class="custom-card" preset="card" title="连接WiFi" size="huge" :bordered='true'>
        ...
        <n-flex vertical size="large">
            ...
            <n-popover trigger="hover">
                <template #trigger>
                    <n-button @click="connect_ap">连接</n-button>
                </template>
                呃呃呃
            </n-popover>
    ...
    <n-list>
        <template #header>
            <n-flex justify="space-between">
                ...
                <n-button strong secondary type="primary" @click="get_wifi_list">刷新</n-button>
            </n-flex>
        </template>
