...
export const connect_device1 = async (message) => {
    ...
    try {
        ...
        if (default_store.wifi_info.host) {
            message.success(t("toast.connect_success"))
        } else {
            message.warning(t("toast.connect_failed"))
        }
    }
}
...
export const connect_device = async () => {
    ...
    if (default_store.wifi_info.enable_custom_address) {
        ...
        fetch_pool.push(get_host("http://wand-esp32/whoami"))
